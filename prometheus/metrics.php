...
foreach ($data["cron_jobs"] as $cron) {
  ...
  $values = [
    "glpi_crontasks_state" => $state,
    "glpi_crontasks_frequency_seconds" => $frequency,
    "glpi_crontasks_frequency_minutes" => $frequency / 60,
    "glpi_crontasks_frequency_hours" => $frequency / 60 / 60,
    "glpi_crontasks_last_run_seconds" => $lastRun,
    "glpi_crontasks_last_run_minutes" => $lastRun / 60,
    "glpi_crontasks_last_run_hours" => $lastRun / 60 / 60,
    "glpi_crontasks_last_run_days" => $lastRun / 60 / 60 / 24,
    "glpi_crontasks_run_state" =>
...
echo "# HELP glpi_major_version Versão major atual do GLPI do rodando\n";
echo "# TYPE glpi_major_version counter\n";
echo "glpi_major_version {$version[0]}\n\n";

echo "# HELP glpi_minor_version Versão minor atual do GLPI do rodando\n";
echo "# TYPE glpi_minor_version counter\n";
echo "glpi_minor_version {$version[1]}\n\n";

echo "# HELP glpi_patch_version Versão patch atual do GLPI do rodando\n";
echo "# TYPE glpi_patch_version counter\n";
echo "glpi_patch_version {$version[2]}\n\n";
